/* ratelimit.js — fixed-window login throttle in D1.
   ...
   peek-before-bump-after-failure design. Scoped to /auth/login only. */
...
async function ipBucket(env, request) {
  const ip = request.headers.get("CF-Connecting-IP") || "unknown";
  const day = new Date().toISOString().slice(0, 10);
  return "li:" + await hmacHex(env.RATE_SALT || "dev-salt", ip + "|" + day);
}
