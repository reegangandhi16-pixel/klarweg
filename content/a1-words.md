# Klarweg A1 — Vocabulary by Chapter (Ch 1 → 47)

Every word from the Netzwerk neu A1 glossary AND the appendix Thematische Wortgruppen, placed under its Klarweg chapter, grouped by tier. Total entries: 1990.

Tier key: **Core Active** = produce from memory · **Active Support** = recognise always, produce sometimes · **Passive** = recognition only · **Reference/System** = grammar/classroom metalanguage.

## Chapter 1 — Das Alphabet & Aussprache  ·  8 words

**Core Active**

- das Alphabet — alphabet
- der Buchstabe — letter
**Active Support**

- buchstabieren — to spell
- mitlesen — to read along
- zuerst — at first
**Reference / System**

- der Konsonant — consonant
- der Umlaut — umlaut
- der Vokal — vowel

## Chapter 2 — Vokale & Laute  ·  16 words

**Active Support**

- achten — to pay attention
- gleich (same) — the same
- langsam — slow
- laut — loud
- noch einmal — once more
**Passive**

- klopfen — to knock
- mitsprechen — to speak along
- nachsprechen — to repeat (after)
**Reference / System**

- betont — stressed
- das Wortinnere — inside of word
- der Silbenanfang — start of syllable
- der Wortanfang — start of word
- die Aussprache — pronunciation
- die Betonung — stress (accent)
- die Satzmelodie — intonation
- doppelt — double

## Chapter 3 — Zahlen 0–100  ·  39 words

**Core Active**

- acht — eight
- achtundzwanzig — twenty-eight
- achtzehn — eighteen
- die Handynummer — mobile number
- die Telefonnummer — telephone number
- die Zahl — number
- drei — three
- dreiundzwanzig — twenty-three
- dreizehn — thirteen
- eins — one
- einundzwanzig — twenty-one
- elf — eleven
- fünf — five
- fünfundzwanzig — twenty-five
- fünfzehn — fifteen
- neun — nine
- neunundzwanzig — twenty-nine
- neunzehn — nineteen
- null — zero
- sechs — six
- sechsundzwanzig — twenty-six
- sechzehn — sixteen
- sieben — seven
- siebenundzwanzig — twenty-seven
- siebzehn — seventeen
- vier — four
- vierundzwanzig — twenty-four
- vierzehn — fourteen
- zehn — ten
- zwanzig — twenty
- zwei — two
- zweiundzwanzig — twenty-two
- zwölf — twelve
**Active Support**

- dann — then
- der Punkt — dot/period
- der Unterstrich — underscore
- die E-Mail-Adresse — email address
- minus — minus
- plus — plus

## Chapter 4 — Hallo!  ·  39 words

**Core Active**

- auf Wiedersehen — goodbye
- bitte — please
- danke — thank you
- das — this/that
- Frau — Mrs/Ms
- gehen — to go
- gute Nacht — good night
- guten Abend — good evening
- guten Morgen — good morning
- guten Tag — good day/hello
- hallo — hello
- Herr — Mr
- ja — yes
- nein — no
- tschüs — bye
**Active Support**

- auch — also
- bis bald — see you soon
- bis dann — until then
- bis später — see you later
- ciao — bye
- da — there
- danke schön — thank you
- ganz — quite/pretty
- gut — good
- hier — here
- okay — okay
- sehr — very
- so — so/so-so
- so (So Herr Wolan) — so
- willkommen — welcome
**Passive**

- grüezi — hello (CH)
- grüß Gott — good day (AT)
- hey — hey
- hi — hi
- hurra — hooray
- moin — hi (N-DE)
**Reference / System**

- die Höflichkeit — politeness
- formell — formal
- informell — informal

## Chapter 5 — Wer bist du?  ·  38 words

**Core Active**

- auf — in/on (lang.)
- aus — from
- dein/deine — your
- der Nachname — last name
- der Name — name
- der Vorname — first name
- du — you
- er — he
- es — it
- heißen — to be called
- ich — I
- ihr — you (pl.)
- in — in
- kommen — to come from
- mein/meine — my
- sein — to be
- Sie — you (formal)
- sie — she/it
- wir — we
- wohnen — to live
**Active Support**

- antworten — to answer
- das Telefon — telephone
- der Partner — partner (m)
- die Antwort — answer
- die Partnerin — partner (f)
- die Person — person
- fragen — to ask
- für — for
- kennen — to know
- vorstellen — to introduce
**Passive**

- der Reiseführer — travel guide (m)
- die Reiseführerin — travel guide (f)
- raten — to guess
- variieren — to vary
**Reference / System**

- das Personalpronomen — personal pronoun
- das Subjekt — subject
- das Verb — verb
- der Aussagesatz — declarative sentence

## Chapter 6 — W-Fragen  ·  34 words

**Core Active**

- warum — why
- was — what
- wer — who
- wie — how/what
- wie viel/viele — how much/many
- wo — where
- woher — from where
**Active Support**

- die Frage — question
- die Gruppe — group
- dritt — threes
- ergänzen — to complete
- genau — exactly
- notieren — to note
**Passive**

- bekannt — well-known
- das Interview — interview
- das Symbol — symbol
- die Bildgeschichte — comic strip
- die Notiz — note
- die Reihenfolge — sequence
- die Situation — situation
- die Tabelle — table/chart
- mischen — to mix/shuffle
- passend — matching
- ähnlich — similar
**Reference / System**

- ankreuzen — to mark/tick
- bilden — to form
- das Fragewort — question word
- das W-Wort — w-word
- die W-Frage — w-question
- kreisen — to circle
- nummerieren — to number
- ordnen — to arrange
- würfeln — to roll dice
- zeichnen — to draw

## Chapter 7 — Ja oder Nein?  ·  20 words

**Core Active**

- hören — to hear
- lesen — to read
- nicht — not
- sagen — to say
- schreiben — to write
- sprechen — to speak
- verstehen — to understand
**Active Support**

- bedeuten — to mean
- das Gespräch — conversation
- der Dank — thanks
- der Dialog — dialogue
- die Entschuldigung — sorry/excuse me
- ein bisschen — a little
**Passive**

- das Wort — word
- die Sprache — language
**Reference / System**

- der Satz — sentence
- die Art — type/manner
- die Ja-/Nein-Frage — yes/no question
- die Weise — manner
- führen — to have (conv.)

## Chapter 8 — Goethe Mini 1  ·  76 words

**Core Active**

- sie (Pl.) — they
**Active Support**

- das Land — country
- der Text — text
- der/die Deutsche — German (person)
- die Stadt — city
- kurz — short
- zu — as (pair)
- zweit — pair
**Passive**

- Algerien — Algeria
- andere — other
- Arabisch — Arabic
- argentinisch — Argentinian
- Brasilien — Brazil
- Bulgarisch — Bulgarian
- China — China
- darauf — upon
- der Irak — Iraq
- der Iran — Iran
- der Jemen — Yemen
- der Libanon — Lebanon
- Deutsch — German (lang.)
- deutsch — German (adj.)
- Deutschland — Germany
- deutschsprachig — German-speaking
- die Muttersprache — mother tongue
- die Niederlande — Netherlands
- die Schweiz — Switzerland
- die Slowakei — Slovakia
- die Tafel — blackboard
- die Türkei — Turkey
- die Ukraine — Ukraine
- die USA — USA
- Dänemark — Denmark
- Englisch — English
- Frankreich — France
- Französisch — French
- gegenseitig — mutual
- Griechenland — Greece
- Indonesisch — Indonesian
- international — international
- Italien — Italy
- Italienisch — Italian
- Japan — Japan
- Japanisch — Japanese
- Kanada — Canada
- Maori — Maori
- Mexiko — Mexico
- Neuseeland — New Zealand
- Polen — Poland
- Polnisch — Polish
- Portugal — Portugal
- Portugiesisch — Portuguese
- reagieren — to react
- Russisch — Russian
- Russland — Russia
- Rätoromanisch — Raeto-Romanic
- Serbisch — Serbian
- Spanien — Spain
- Spanisch — Spanish
- Syrien — Syria
- Südamerika — South America
- Thai — Thai
- Thailand — Thailand
- Tunesien — Tunisia
- Türkisch — Turkish
- Ungarisch — Hungarian
- Ungarn — Hungary
- Österreich — Austria
- über — about
- über (+A) — about
**Reference / System**

- das Befinden — condition
- das Redemittel — useful phrase
- die Grammatik — grammar
- grüßen — to greet
- tauschen — to swap
- verabschieden — to say goodbye

## Chapter 9 — Verben im Alltag  ·  22 words

**Core Active**

- arbeiten — to work
- brauchen — to need
- kochen — to cook
- lernen — to learn
- machen — to do/make
- spielen — to play
- und — and
**Active Support**

- das Wochenende — weekend
- morgen — tomorrow
- sammeln — to collect
- wirklich — really
**Passive**

- das Deutschbuch — German book
- das Kapitel — chapter
- das Übungsbuch — exercise book
- die Form — form/shape
- die Lernkarte — flashcard
- die Seite — page
- nennen — to name
**Reference / System**

- der Infinitiv — infinitive
- der Lernwortschatz — vocabulary list
- die Endung — ending
- die Regel — rule

## Chapter 10 — Der / Die / Das  ·  71 words

**Core Active**

- das Auto — car
- das Buch — book
- das/der/die — the (article)
- der Computer — computer
- der Stift — pen/pencil
- der Tisch — table
- die — the (pl.)
- die Straße — street
- ein/eine — a/an
**Active Support**

- das Beispiel — example
- das Foto — photo
- das Wörterbuch — dictionary
- der Schlüssel — key
- die Karte — card
- leer — empty
- merken — to remember
- noch — yet/any
**Passive**

- das Artikelbild — article picture
- das Butterbrot — sandwich
- das Experiment — experiment
- das Gästebuch — guestbook
- das Handtuch — towel
- das Heft — exercise book
- das Kursplakat — course poster
- das Paar — pair
- das Plakat — poster
- das Würstchen — sausage
- das Würstel — sausage (AT)
- der Bericht — report
- der Blog — blog
- der Eintrag — entry
- der Kindergarten — kindergarten
- der Koffer — suitcase
- der Stapel — deck/pile
- der/die Kranke — sick person
- die Anzeige — advert
- die Assoziation — association
- die Autobahn — highway
- die Flasche — bottle
- die Homepage — homepage
- die Methode — method
- die Mindmap — mind map
- die Nudel — noodle
- die Zeichnung — drawing
- eigene/eigener — own
- einfallen — to think of
- recherchieren — to research
- thematisch — thematic
- verwenden — to use
- weitere — further
- ziehen — to draw (card)
**Reference / System**

- berichten — to report
- bestimmt — definite
- das Nomen — noun
- das Wortende — word ending
- der Artikel — article (gram.)
- der Plural — plural
- der Singular — singular
- der Wortteil — word part
- die Pluralendung — plural ending
- die Pluralform — plural form
- die Wortgruppe — word group
- feminin — feminine
- markieren — to highlight
- maskulin — masculine
- neutrum — neuter
- unbestimmt — indefinite
- unterstreichen — to underline
- vergleichen — to compare
- zuordnen — to match
- zusammengehören — to fit together

## Chapter 11 — Kein & Nicht  ·  12 words

**Core Active**

- die Zeit — time
- heute — today
- jetzt — now
- kein/keine — no/not a
**Active Support**

- aber — but
- das Problem — problem
- mehr — more/anymore
- nicht mehr — not anymore
- oder — or
**Passive**

- das Glück — luck
- schon — already
**Reference / System**

- der Negationsartikel — negation article

## Chapter 12 — Akkusativ  ·  15 words

**Core Active**

- den — the (acc.)
- finden — to find
- haben — to have
- suchen — to look for
**Active Support**

- alles — everything
- das Geschäft — shop
- die Lösung — solution
- die Sache — thing
- ein paar — a few
- genug — enough
- so (Deutsch) — this way
**Passive**

- das Ding — thing
- das Meer — sea
**Reference / System**

- der Akkusativ — accusative
- der Nominativ — nominative

## Chapter 13 — Adjektive mit sein  ·  76 words

**Core Active**

- falsch — wrong
- groß — big
- interessant — interesting
- klein — small
- richtig — correct
- schön — nice/pretty
- super — super
- toll — great
- viel/viele — a lot/many
**Active Support**

- als (than) — than
- alt — old
- anstrengend — exhausting
- attraktiv — attractive
- beide — both
- beliebt — popular
- berühmt — famous
- beschreiben — to describe
- besser — better
- billig — cheap
- böse — angry
- cool — cool
- der Mensch — person
- doof — silly/dumb
- einfach (simply) — simply
- faul — lazy
- fleißig — hardworking
- freundlich (Chef) — friendly
- froh — glad
- ganz (whole) — whole
- gefährlich — dangerous
- genau (where) — exactly
- glücklich — happy
- günstig — cheap/good value
- hoch — high
- hungrig — hungry
- hässlich — ugly
- jung — young
- kalt — cold
- kaputt — broken
- lang — long
- langweilig — boring
- leer (Kühlschrank) — empty
- leicht — easy
- lustig — funny
- müde — tired
- nett — nice
- neu — new
- normal — normal
- perfekt — perfect
- praktisch — practical
- richtig (really) — really
- sauber — clean
- schnell — quick
- schwer (difficult) — difficult
- schwer (heavy) — heavy
- sicher — sure/safe
- teuer — expensive
- warm — warm
- wichtig — important
- wunderbar — wonderful
- wunderschön — beautiful
**Passive**

- breit — wide
- der Mann (boy) — boy/man (excl.)
- eilig — in a hurry
- elegant — elegant
- fehlend — missing
- ideal — ideal
- individuell — individual
- k.o. — exhausted
- lebendig — lively
- originell — original
- populär — popular
- supernett — super nice
- top — top/great
- typisch — typical
**Reference / System**

- das Adjektiv — adjective

## Chapter 14 — Essen & Trinken  ·  82 words

**Core Active**

- das Brot — bread
- das Brötchen — bread roll
- das Ei — egg
- das Fleisch — meat
- das Gemüse — vegetable
- das Obst — fruit
- das Wasser — water
- der Apfel — apple
- der Fisch — fish
- der Kaffee — coffee
- der Kuchen — cake
- der Käse — cheese
- der Saft — juice
- der Salat — salad
- der Tee — tea
- der Zucker — sugar
- die Banane — banana
- die Kartoffel — potato
- die Milch — milk
- die Schokolade — chocolate
- die Tomate — tomato
- essen — to eat
- gern — gladly/like to
- gerne — with pleasure
- möchten — would like
- mögen — to like
- trinken — to drink
**Active Support**

- das Abendessen — dinner
- das Frühstück — breakfast
- das Getränk — drink
- das Lebensmittel — groceries
- das Mittagessen — lunch
- das Müsli — muesli
- das Salz — salt
- das Öl — oil
- der Apfelsaft — apple juice
- der Durst — thirst
- der Hunger — hunger
- der Joghurt — yogurt
- der Orangensaft — orange juice
- der Pfeffer — pepper
- der Reis — rice
- der Schinken — ham
- die Butter — butter
- die Cola — cola
- die Gurke — cucumber
- die Limonade — lemonade
- die Marmelade — jam
- die Sahne — cream
- die Suppe — soup
- die Süßigkeit — sweet
- die Wurst — sausage
- frisch — fresh
- lecker — tasty
- probieren — to try
- schmecken — to taste
- süß — sweet
**Passive**

- asiatisch — Asian
- das Dessert — dessert
- das Fett — fat
- das Fischgericht — fish dish
- das Hähnchen — chicken
- das Sushi — sushi
- der Champignon — mushroom
- der Döner — kebab
- der Emmentaler — Emmental cheese
- der Essig — vinegar
- der Keks — cookie
- der Stollen — fruit loaf
- die Birne — pear
- die Chips — crisps
- die Olive — olive
- die Pizza — pizza
- die Pommes frites — chips
- die Spaghetti — spaghetti
- die Spezialität — speciality
- die Zwiebel — onion
- satt — full
- schneiden — to cut
- schälen — to peel
- zubereiten — to prepare
**Reference / System**

- die Vorliebe — preference

## Chapter 15 — Im Café  ·  67 words

**Core Active**

- bestellen — to order
- bezahlen — to pay
- das Café — café
- das Essen — meal/food
- das Restaurant — restaurant
- der Euro — euro
- der Preis — price
- die Rechnung — bill
- die Speisekarte — menu
- kaufen — to buy
- kosten — to cost
- nehmen — to take
- zahlen — to pay
**Active Support**

- bringen — to bring
- das Gericht — dish/meal
- das Glas — glass
- das Messer — knife
- das Stück — piece
- das Trinkgeld — tip
- der Cent — cent
- der Kellner — waiter
- der Löffel — spoon
- der Teller — plate
- die Bestellung — order
- die Gabel — fork
- die Kellnerin — waitress
- die Serviette — napkin
- die Tasse — cup
- etwas — anything/some
- getrennt — separate
- gleichfalls — likewise
- guten Appetit — enjoy!
- natürlich — of course
- natürlich (Antwort) — of course
- sonst — otherwise
- stimmen — to be true
- stimmen (Stimmt so) — keep the change
- wechseln — to change
- zusammen — together
**Passive**

- Co (& Co) — co
- das Beisl — pub (AT)
- das Eis — ice cream
- das Kaffeehaus — coffee house
- das Lokal — restaurant
- das Schnitzel — schnitzel
- der Biergarten — beer garden
- der Einkaufswagen — shopping cart
- der Einkaufszettel — shopping list
- der Franken — Swiss franc
- der Kassenzettel — receipt
- der/das Sandwich — sandwich
- die Apfelsaftschorle — apple spritzer
- die Apfelschorle — apple spritzer
- die Bar — bar
- die Beiz — pub (CH)
- die Kneipe — pub
- die Kosten — costs
- die Mahlzeit — meal/enjoy
- die Pommes — chips
- die Salami — salami
- die Selbstbedienung — self-service
- die Strandbar — beach bar
- die Tomatensuppe — tomato soup
- drankommen — to have one's turn
- prost — cheers
- zum Wohl — cheers
**Reference / System**

- der Imperativ — imperative

## Chapter 16 — Hobbies & Freizeit  ·  115 words

**Core Active**

- das Hobby — hobby
- der Sport — sport
- die Freizeit — free time
- fotografieren — to photograph
- Fußball — football
- joggen — to jog
- klettern — to climb
- lieben — to love
- schwimmen — to swim
- singen — to sing
- tanzen — to dance
- Tennis — tennis
- wandern — to hike
**Active Support**

- Basketball — basketball
- das Angebot (Kultur) — offer (cultural)
- das Event — event
- das Fest — festival/party
- das Festival — festival
- das Fitness-Studio — gym
- das Fußballspiel — football match
- das Kino — cinema
- das Konzert — concert
- das Mal — time (occurrence)
- das Museum — museum
- das Programm — programme
- das Schwimmbad — swimming pool
- das Skifahren — skiing
- das Spiel — game
- das Stadion — stadium
- das Theater — theatre
- das Ticket — ticket
- das Training — practice
- das Video — video
- der Ausflug — excursion
- der Ball — ball
- der Eintritt — entrance/admission
- der Film — film
- der Freund — friend (m)
- der Ski — ski
- der Spaziergang — walk
- der Spaß — fun
- die Aktivität — activity
- die Freizeitaktivität — leisure activity
- die Freundin — friend (f)
- die Karte (Konzert) — ticket
- die Kunst — art
- die Leute — people
- die Musik — music
- die Veranstaltung — event
- feiern — to celebrate
- Karate — karate
- laufen — to run
- malen — to paint
- reisen — to travel
- spazieren gehen — to go for a walk
- trainieren — to train
- Yoga — yoga
- zeigen (Film) — to show
**Passive**

- chillen — to chill
- das Double Feature — double feature
- das Festspiel — festival production
- das Jahrestreffen — yearly meeting
- das Karussell — carousel
- das Kulturfestival — culture festival
- das Lied — song
- das Open-Air-Kino — open-air cinema
- das Orchester — orchestra
- das Picknick — picnic
- das Publikum — audience
- das Ratebild — picture puzzle
- das Requiem — requiem
- das Saxofon — saxophone
- das Sommerfest — summer party
- das Theater-Festival — theatre festival
- der Beginn — beginning
- der Chor — choir
- der Club — club
- der Comic — comic
- der Filmfan — film fan
- der Geigenunterricht — violin lessons
- der Halbmarathon — half marathon
- der Kommentar — comment
- der Konzertbeginn — concert start
- der Marathon — marathon
- der Moment — moment
- der Solist — soloist (m)
- der Sportclub — sports club
- der Star — star
- der Tennisplatz — tennis court
- der Trompetenunterricht — trumpet lessons
- der Zuschauer — spectator (m)
- die Bühne — stage
- die Fahrradtour — bike ride
- die Geige — violin
- die Grillparty — barbecue
- die Konzertkarte — concert ticket
- die Kultur-Nacht — cultural night
- die Museumsnacht — museum night
- die Musikgruppe — music group
- die Musikschule — music school
- die Oper — opera
- die Pantomime — pantomime
- die Produktion — production
- die Sensation — sensation
- die Solistin — soloist (f)
- die Trompete — trumpet
- die Zuschauerin — spectator (f)
- dirigieren — to conduct
- Extra- — extra
- genießen — to enjoy/savour
- glauben — to believe
- grillen — to barbecue
- rund um — all about
- segeln — to sail
- Zumba — zumba

## Chapter 17 — Goethe Mini 2  ·  23 words

**Core Active**

- die Adresse — address
- die E-Mail — email
**Active Support**

- ausfüllen — to fill in
- das Formular — form
- der Familienname — family name
- der Kontakt — contact
- der Wohnort — place of residence
- die Daten — data/files
- die Firma — company
- die Hausnummer — house number
- die Information — information
- die Mail — mail
- die Postleitzahl — postcode
- die Schule — school
- männlich — male
- persönlich — personal
- weiblich — female
- wählen — to choose
**Passive**

- das Mitglied — member
- das Sprachinstitut — language institute
- die Angabe — detail/info
- die Sprachschule — language school
- zusammenpassen — to match

## Chapter 18 — Unregelmäßige Verben  ·  14 words

**Core Active**

- fahren — to drive/go
- geben — to give
- geben (es gibt) — there is
- schlafen — to sleep
- sehen — to see
- treffen — to meet
**Active Support**

- besuchen — to visit
- die Bibliothek — library
- die Mensa — canteen
- die Uni — uni
- waschen — to wash
**Passive**

- das Unicafé — uni café
- die Zeitung — newspaper
**Reference / System**

- unregelmäßig — irregular

## Chapter 19 — Satzposition  ·  36 words

**Core Active**

- abends — in the evening
- duschen — to shower
- frühstücken — to have breakfast
- immer — always
- manchmal — sometimes
- morgens — in the morning
- oft — often
**Active Support**

- allein — alone
- der Alltag — everyday life
- der Mittag — noon
- der Morgen — morning
- der Nachmittag — afternoon
- der Vormittag — morning
- die Nacht — night
- früh — early
- jede/jeder — every
- lange — long (time)
- meistens — mostly
- mittags — at noon
- nachmittags — afternoons
- nachts — at night
- nichts — nothing
- nie — never
- normalerweise — normally
- nur — only
- regelmäßig — regularly
- selten — rarely
- täglich — daily
- vormittags — before noon
- wenig — a little
- zu Hause — at home
- zu Mittag essen — to have lunch
**Passive**

- abwechselnd — alternating
- wach — awake
**Reference / System**

- die Position — position
- die Zeitangabe — time expression

## Chapter 20 — Modalverben 1  ·  18 words

**Core Active**

- können — can
- müssen — must
- wollen — to want
**Active Support**

- der Stress — stress
- die Hausaufgabe — homework
- die Idee — idea
- die Party — party
- hoffentlich — hopefully
- leider — unfortunately
- nächste/nächster — next
- planen — to plan
- stressig — stressful
**Passive**

- der Sprachkurs — language course
- schade — a pity
- verschieden — different
**Reference / System**

- das Modalverb — modal verb
- das Satzende — sentence end
- die Satzklammer — sentence bracket

## Chapter 21 — Uhrzeit  ·  44 words

**Core Active**

- am — on/at
- das Viertel — quarter
- der Dienstag — Tuesday
- der Donnerstag — Thursday
- der Freitag — Friday
- der Mittwoch — Wednesday
- der Montag — Monday
- der Samstag — Saturday
- der Sonntag — Sunday
- der Tag — day
- der Termin — appointment
- die Uhrzeit — time of day
- die Woche — week
- halb — half
- Uhr — o'clock
- um — at (time)
- von … bis — from … to
**Active Support**

- bis — until
- der Abend — evening
- der Kalender — calendar
- der Wochentag — weekday
- die Verabredung — appointment/date
- dienstags — on Tuesdays
- donnerstags — on Thursdays
- freitags — on Fridays
- kurz (time) — just (after)
- mittwochs — on Wednesdays
- montags — on Mondays
- nach (+D, time) — past (time)
- samstags — on Saturdays
- sonntags — on Sundays
- spät — late
- verabreden (sich) — to make plans
- vor (+D, time) — to (before time)
- wann — when
- wie lange — how long
**Passive**

- der Samstagabend — Saturday evening
- der Sonntagnachmittag — Sunday afternoon
- die halbe Stunde — half an hour
- die Minute — minute
- die Sekunde — second
- inoffiziell — informal (time)
- offiziell — official
**Reference / System**

- die Tageszeit — time of day

## Chapter 22 — Berufe & Arbeit  ·  135 words

**Core Active**

- das Büro — office
- der Arzt — doctor (m)
- der Beruf — profession
- der Lehrer — teacher (m)
- der Student — student (m)
- der Urlaub — holiday
- die Arbeit — work
- die Arbeitszeit — working hours
- die Lehrerin — teacher (f)
- die Studentin — student (f)
- die Stunde — hour
- die Ärztin — doctor (f)
**Active Support**

- als — as
- arbeitslos — unemployed
- beantworten — to answer
- das Geld — money
- das Praktikum — internship
- das Team — team
- der Arbeitsalltag — workday
- der Arbeitsplatz — workplace
- der Architekt — architect (m)
- der Chef — boss (m)
- der Feierabend — end of work day
- der Friseur — hairdresser (m)
- der Ingenieur — engineer (m)
- der Job — job
- der Koch — cook (m)
- der Kollege — colleague (m)
- der Krankenpfleger — nurse (m)
- der Kurs — course
- der Mitarbeiter — co-worker (m)
- der Taxifahrer — taxi driver (m)
- der Verkäufer — salesperson (m)
- die Architektin — architect (f)
- die Bewerbung — job application
- die Chefin — boss (f)
- die Friseurin — hairdresser (f)
- die Ingenieurin — engineer (f)
- die Kollegin — colleague (f)
- die Krankenpflegerin — nurse (f)
- die Köchin — cook (f)
- die Mitarbeiterin — co-worker (f)
- die Mittagspause — lunch break
- die Pause — break
- die Stelle — position/job
- die Taxifahrerin — taxi driver (f)
- die Verkäuferin — salesperson (f)
- freihaben — to have off
- geben (Unterricht) — to give (lessons)
- pro — per
- verdienen — to earn
**Passive**

- betreuen — to supervise
- das Atelier — studio
- das Job-Portal — job portal
- das Krankenhaus — hospital
- das Labor — laboratory
- das Profil — profile
- das Seminar — seminar
- der Designer — designer (m)
- der Elektriker — electrician (m)
- der Erzieher — childcare worker (m)
- der Fotograf — photographer (m)
- der Handwerker — craftsman
- der Hausmeister — caretaker (m)
- der Hotelchef — hotel manager (m)
- der Händler — merchant (m)
- der Informatiker — IT specialist (m)
- der Journalist — journalist (m)
- der Jurist — lawyer (m)
- der Kundenbesuch — client visit
- der Kursraum — course room
- der Künstler — artist (m)
- der Laborant — lab assistant (m)
- der Mechaniker — mechanic (m)
- der Modedesigner — fashion designer (m)
- der Notarzt — emergency doctor (m)
- der Physiotherapeut — physiotherapist (m)
- der Polizist — police officer (m)
- der Professor — professor (m)
- der Regisseur — director (m)
- der Saison-Job — seasonal job
- der Schauspieler — actor
- der Segellehrer — sailing instructor (m)
- der Sekretär — secretary (m)
- der Sportler — athlete (m)
- der Statist — extra (m)
- der Techniker — technician (m)
- der Therapeut — therapist (m)
- die Agentur für Arbeit — employment office
- die Baustelle — construction site
- die Computerarbeit — computer work
- die Computerfirma — computer company
- die Designerin — designer (f)
- die Elektrikerin — electrician (f)
- die Erzieherin — childcare worker (f)
- die Fernsehproduktion — TV production
- die Filmproduktion — film production
- die Fotografin — photographer (f)
- die Handwerkerin — craftswoman
- die Hausmeisterin — caretaker (f)
- die Hotelchefin — hotel manager (f)
- die Händlerin — merchant (f)
- die Informatikerin — IT specialist (f)
- die Journalistin — journalist (f)
- die Juristin — lawyer (f)
- die Kantine — canteen
- die Karriere — career
- die Kursstatistik — class statistic
- die Künstlerin — artist (f)
- die Laborantin — lab assistant (f)
- die Mechanikerin — mechanic (f)
- die Modedesignerin — fashion designer (f)
- die Modeschule — fashion school
- die Musikproduktion — music production
- die Möglichkeit — possibility
- die Notärztin — emergency doctor (f)
- die Physiotherapeutin — physiotherapist (f)
- die Polizistin — police officer (f)
- die Professorin — professor (f)
- die Präsentation — presentation
- die Regisseurin — director (f)
- die Schauspielerin — actress
- die Segellehrerin — sailing instructor (f)
- die Sekretärin — secretary (f)
- die Sportlerin — athlete (f)
- die Statistik — statistics
- die Statistin — extra (f)
- die Technikerin — technician (f)
- die Therapeutin — therapist (f)
- die Universität — university
- die Werkstatt — workshop
- kreativ — creative
- länger — longer
- mehrere — several
- präsentieren — to present

## Chapter 23 — Possessivartikel & Familie  ·  43 words

**Core Active**

- das Kind — child
- der Bruder — brother
- der Mann — man/husband
- der Sohn — son
- der Vater — father
- die Eltern — parents
- die Familie — family
- die Frau — woman/wife
- die Mutter — mother
- die Schwester — sister
- die Tochter — daughter
- euer/eure — your (pl.)
- Ihr/Ihre — your (formal)
- ihr/ihre — its/her
- sein/seine — his
- unser/unsere — our
**Active Support**

- das Baby — baby
- das Mädchen — girl
- der Ehemann — husband
- der Großvater — grandfather
- der Hund — dog
- der Junge — boy
- der Nachbar — neighbour (m)
- der Onkel — uncle
- der Opa — grandpa
- der/die Bekannte — acquaintance
- der/die Jugendliche — teenager
- die Ehefrau — wife
- die Geschwister — siblings
- die Großeltern — grandparents
- die Großmutter — grandmother
- die Katze — cat
- die Nachbarin — neighbour (f)
- die Oma — grandma
- die Tante — aunt
- ledig — single
- verheiratet — married
**Passive**

- das Familienfoto — family photo
- der Hamster — hamster
- der/die Erwachsene — adult
- der/die Verwandte — relative
- die Fantasie — imagination
- die Maus — mouse
**Reference / System**

- der Possessivartikel — possessive article

## Chapter 24 — Datum & Ordinalzahlen  ·  66 words

**Core Active**

- achtzig — eighty
- das Datum — date
- das Jahr — year
- der April — April
- der August — August
- der Dezember — December
- der Februar — February
- der Geburtstag — birthday
- der Januar — January
- der Juli — July
- der Juni — June
- der Mai — May
- der Monat — month
- der März — March
- der November — November
- der Oktober — October
- der September — September
- die Einladung — invitation
- dreißig — thirty
- einhundert — one hundred
- eintausend — one thousand
- fünfzig — fifty
- hundert — hundred
- neunzig — ninety
- sechzig — sixty
- siebzig — seventy
- tausend — thousand
- vierzig — forty
**Active Support**

- ab — from (number)
- alle — all
- das Geschenk — present
- der Frühling — spring
- der Glückwunsch — congratulations
- der Herbst — autumn
- der Sommer — summer
- der Winter — winter
- der Wunsch — wish
- der/die Erste — first
- die Blume — flower
- die Feier — celebration
- die Jahreszeit — season
- die Milliarde — billion
- die Million — million
- die Überraschung — surprise
- dreimal — three times
- dreitausend — three thousand
- erste/erster — first
- insgesamt — in total
- mindestens — at least
- rund (around) — around/about
- schenken — to give (gift)
- ungefähr — approximately
- wünschen — to wish
- zweihundert — two hundred
- zweimal — twice
**Passive**

- (ein)hunderttausend — one hundred thousand
- circa — approx.
- das Geburtsdatum — date of birth
- der Geburtsort — place of birth
- der Kilometer — kilometre
- der Meter — metre
- der Überraschungstag — surprise day
- die Einweihungsfeier — housewarming
- zehntausend — ten thousand
- zweihunderttausend — two hundred thousand
**Reference / System**

- die Ordinalzahl — ordinal number

## Chapter 25 — Trennbare Verben  ·  31 words

**Core Active**

- anrufen — to call
- aufstehen — to get up
- einkaufen — to shop
- einladen — to invite
- fernsehen — to watch TV
- mitkommen — to come along
**Active Support**

- abholen — to pick up
- anfangen — to start
- anmachen — to turn on
- aufhören — to stop
- aufmachen — to open
- aufpassen — to pay attention
- ausmachen — to turn off
- beginnen — to begin
- einkaufen gehen — to go shopping
- einpacken — to pack
- losgehen — to start/set off
- mitbringen — to bring
- mitmachen — to join in
- mitnehmen — to take along
- rausgehen — to go out
- weitermachen — to continue
- zumachen — to close
**Passive**

- aufhängen — to hang up
- aufschreiben — to write down
- aufstellen — to arrange
- einsammeln — to collect in
- vorbereiten — to prepare
- vorlesen — to read aloud
- wegräumen — to put away
**Reference / System**

- trennbar — separable

## Chapter 26 — Goethe Mini 3  ·  33 words

**Core Active**

- wissen — to know
**Active Support**

- da sein — to be there
- der Gast — guest
- der Kunde — customer (m)
- die Besprechung — meeting
- die Kundin — customer (f)
- die Praxis — surgery/practice
- die Verspätung — delay
- entschuldigen — to excuse
- fertig — ready
- frei — free
- frei (Termin) — available
- gleich — nearly/soon
- gut (Schon gut) — alright
- haben (Termin) — to make (appt.)
- höflich — polite
- leidtun — to be sorry
- machen (Macht nichts) — to matter
- pünktlich — on time
- tun — to do
- vereinbaren — to arrange
- vielleicht — maybe
- warten — to wait
- zu (spät) — too (late)
- überlegen — to think about
**Passive**

- abwechseln — to take turns
- der Treffpunkt — meeting point
- die Achtung — careful
- die Pünktlichkeit — punctuality
- die Rollenkarte — role card
- um (+A) — for (forgiveness)
- unhöflich — impolite
- vorspielen — to act out

## Chapter 27 — Präteritum haben/sein  ·  5 words

**Active Support**

- der Besuch — visit
- früher — formerly
- gestern — yesterday
- vorgestern — day before yesterday
**Reference / System**

- das Präteritum — simple past

## Chapter 28 — Perfekt Einführung  ·  4 words

**Core Active**

- haben (gelernt) — to have (aux)
**Active Support**

- passieren — to happen
**Reference / System**

- das Partizip — participle
- das Perfekt — perfect tense

## Chapter 29 — Perfekt mit haben  ·  6 words

**Active Support**

- das Projekt — project
- das Semester — semester
- das Studium — studies
- der Unterricht — lessons
- die Prüfung — exam
**Passive**

- studieren — to study

## Chapter 30 — Perfekt mit sein  ·  20 words

**Core Active**

- bleiben — to stay
- der Flug — flight
- die Reise — trip
- fliegen — to fly
- sein (passiert) — to be (happen)
**Active Support**

- abfahren — to depart
- ankommen — to arrive
- ausgehen — to go out
- das Angebot (offer) — offer
- das Ausland — abroad
- laufen (läuft gut) — to go (well)
- losfahren — to set off
- nehmen (Urlaub) — to take (holiday)
- werden — to become/turn
- wirklich (war es) — really
- zurück — back
- zurückfahren — to drive back
**Passive**

- einschlafen — to fall asleep
- wecken — to wake up
**Reference / System**

- die Ortsveränderung — change of location

## Chapter 31 — Partizip II mit Präfix  ·  11 words

**Core Active**

- bekommen — to get/receive
**Active Support**

- annehmen — to accept
- der Teil — part
- empfehlen — to recommend
- erklären — to explain
- erzählen — to tell
- vergessen — to forget
- verkaufen — to sell
**Passive**

- entdecken — to discover
- vermissen — to miss
**Reference / System**

- das Präfix — prefix

## Chapter 32 — Akkusativpronomen  ·  8 words

**Core Active**

- dich — you (acc.)
- euch — you (pl. acc.)
- ihn — him (acc.)
- mich — me (acc.)
- sie (für sie) — her (acc.)
- uns — ours/us
**Active Support**

- wen — whom
**Reference / System**

- das Pronomen — pronoun

## Chapter 33 — Sätze verbinden  ·  10 words

**Core Active**

- denn (because) — because
**Active Support**

- also — so/therefore
- der Grund — reason
- doch — on the contrary
- nicht nur — not only
**Passive**

- durch — through
- durch (+A, Portal) — through
- ebenso — likewise/also
**Reference / System**

- das Ereignis — event/result
- die Aussage — statement

## Chapter 34 — Schreiben & Einladungen  ·  22 words

**Core Active**

- die Nachricht — message
**Active Support**

- an (+A, schreiben) — to (write to)
- denken — to think
- der Brief — letter
- der Gruß — greeting
- der Vertrag — contract
- die Anrede — salutation
- die Dame — madam
- freundlich — friendly/sincerely
- geehrt — honoured
- herzlich — warm (regards)
- lieb — kind/dear
- unterschreiben — to sign
**Passive**

- der Absender — sender
- der Betreff — subject (email)
- der Briefstandard — letter standard
- der Empfänger — recipient (m)
- die Einladungs-Mail — invitation email
- die Empfängerin — recipient (f)
- die Unterschrift — signature
**Reference / System**

- die Anredeformel — salutation formula
- die Grußformel — closing formula

## Chapter 35 — Goethe Mini 4  ·  3 words

**Passive**

- der Reisebericht — travelogue
- die Urlaubsnachricht — holiday message
- tatsächlich — actually

## Chapter 36 — Dativ Einführung  ·  3 words

**Core Active**

- dem — the (dat.)
- wem — to whom
**Reference / System**

- der Dativ — dative

## Chapter 37 — Präpositionen mit Dativ  ·  23 words

**Core Active**

- bei — at/with
- das Fahrrad — bicycle
- der Bus — bus
- der Zug — train
- mit — with
- nach — about/after
- nach (+D) — to
- seit — since/for
- von — by/from
- zu (+D) — to
**Active Support**

- das Flugzeug — airplane
- das Schiff — ship
- das Taxi — taxi
- die S-Bahn — suburban train
- die Straßenbahn — tram
- die Tram — tram
- die U-Bahn — subway
- gegenüber — across from
- zu (+D, Kunden) — to (people)
- zu Fuß — on foot
- zu Fuß gehen — to go on foot
**Passive**

- das Motorrad — motorcycle
**Reference / System**

- die Präposition — preposition

## Chapter 38 — Wechselpräpositionen  ·  132 words

**Core Active**

- an (+D, sitzen) — at
- auf (+D) — on
- blau — blue
- das Bad — bathroom
- das Bett — bed
- das Haus — house
- das Schlafzimmer — bedroom
- das Sofa — sofa
- das Wohnzimmer — living room
- das Zimmer — room
- der Bahnhof — train station
- der Schrank — cupboard
- der Stuhl — chair
- die Küche — kitchen
- die Wohnung — flat
- gelb — yellow
- grau — grey
- grün — green
- hinter (+D) — behind
- in (+D) — in
- neben (+D) — next to
- rot — red
- schwarz — black
- unter (+D) — under
- vor (+D) — before
- weiß — white
- wohin — where to
- zwischen (+D) — between
- über (+D) — over/above
**Active Support**

- ansehen — to look at
- aus (+D, Holz) — made of
- besondere — special
- besonders — special/especially
- besonders (especially) — especially
- braun — brown
- daneben — beside it
- das Arbeitszimmer — study room
- das Bild — picture
- das Erdgeschoss — ground floor
- das Fenster — window
- das Gerät — device
- das Hochhaus — tower block
- das Kinderzimmer — children's room
- das Licht — light
- das Regal — shelf
- der Balkon — balcony
- der Baum — tree
- der Blick — view
- der Fernseher — TV set
- der Flur — hallway
- der Fluss — river
- der Garten — garden
- der Herd — stove
- der Kühlschrank — fridge
- der Markt — market
- der Park — park
- der Platz — room/space
- der Platz (square) — square
- der Raum — room/space
- der Schreibtisch — desk
- der See — lake
- der Sessel — armchair
- der Stock — floor/storey
- der Teppich — carpet
- die Bank (Sitz) — bench
- die Dusche — shower
- die Farbe — colour
- die Hauptstadt — capital city
- die Kaffeemaschine — coffee machine
- die Kirche — church
- die Lampe — lamp
- die Lieblingsfarbe — favourite colour
- die Mitte — middle
- die Möbel — furniture
- die Pflanze — plant
- die Spülmaschine — dishwasher
- die Terrasse — patio
- die Toilette — toilet
- die Treppe — stairs
- die Tür — door
- die Waschmaschine — washing machine
- draußen — outside
- dunkel — dark
- gemütlich — cosy
- hell — light/bright
- liegen — to lie/be located
- lila — purple
- oben — above/upstairs
- orange — orange
- sauber machen — to clean
- sitzen — to sit
- stehen — to stand
- stellen — to put/ask
- stellen (put) — to put
- unten — below/downstairs
- voll — full
- zeigen — to point/show
- ziehen (umziehen) — to move
- zufrieden — satisfied
**Passive**

- das Fachwerkhaus — timber house
- das Geschirr — crockery
- das Holz — wood
- das Konzerthaus — concert hall
- das Lieblingszimmer — favourite room
- das Loft — loft
- das Rathaus — town hall
- das Reihenhaus — terraced house
- der Aufzug — elevator
- der Besucher — visitor (m)
- der Bewohner — resident (m)
- der Hafen — harbour
- der Hof — courtyard
- der Lieblingsort — favourite place
- der Spielplatz — playground
- der Stadtpark — city park
- der Turm — tower
- der Weihnachtsmarkt — Christmas market
- die Altbauwohnung — old-building flat
- die Altstadt — old town
- die Architektur — architecture
- die Ausstellung — exhibition
- die Bauzeit — construction period
- die Besucherin — visitor (f)
- die Bewohnerin — resident (f)
- die Brücke — bridge
- die Decke — ceiling/blanket
- die Kiste — box/crate
- die Kunsthalle — art gallery
- die Trendstadt — trendy city
- die Wohnform — type of housing
- renoviert — renovated
**Reference / System**

- die Wechselpräposition — two-way preposition

## Chapter 39 — Ortsangaben & Kurzformen  ·  116 words

**Core Active**

- am — at (place)
- der Flughafen — airport
- der Supermarkt — supermarket
- der Weg — way
- die Apotheke — pharmacy
- die Bank (Geld) — bank
- die Haltestelle — stop
- die Post — post office
- die Wegbeschreibung — directions
- geradeaus — straight ahead
- links — left
- nach Hause — (toward) home
- rechts — right
**Active Support**

- aussteigen — to get off
- bar — in cash
- benutzen — to use
- da vorne — over there
- das Apartment — apartment
- das Doppelzimmer — double room
- das Dorf — village
- das Einzelzimmer — single room
- das Gepäck — luggage
- das Gleis — track
- das Hotel — hotel
- das Paket — package
- das Reiseziel — destination
- das Schild — sign
- das Zentrum — centre
- der Ausgang — exit
- der Ausweis — ID card
- der Badeurlaub — beach holiday
- der Bahnsteig — platform
- der Bäcker — baker
- der Ort — place/village
- der Pass — passport
- der Plan — plan
- der Reiseführer (Buch) — guidebook
- der Stadturlaub — city break
- der Strand — beach
- der Urlaubsort — holiday resort
- der Zoo — zoo
- die Abfahrt — departure
- die Ankunft — arrival
- die Anmeldung — registration
- die Anmeldung (registr.) — registration
- die Auskunft — information
- die Bäckerei — bakery
- die Ecke — corner
- die Fahrkarte — ticket
- die Insel — island
- die Jugendherberge — youth hostel
- die Karte (map) — map
- die Karte (pay) — card
- die Kreditkarte — credit card
- die Lage — location
- die Miete — rent
- die Nummer — number
- die Nähe — proximity
- die Papiere — documents
- die Postkarte — postcard
- die Rezeption — reception
- die Route — route
- die Sehenswürdigkeit — sight/attraction
- die Stadtführung — city tour
- die Station — station/stop
- die Städtereise — city trip
- direkt — right/directly
- dort — there
- einfach — easy
- einsteigen — to board
- etwas (a bit) — a bit
- nehmen (Bus) — to take (bus)
- offen — open
- reservieren — to book/reserve
- umsteigen — to change (transport)
- umziehen — to move (house)
- unterwegs — on the way
- verpassen — to miss (train)
- vorne — in front
- weit (far) — far
- zentral — central
- übernachten — to stay overnight
**Passive**

- besichtigen — to view
- das Konto — account
- das Souvenir — souvenir
- der Automat — vending machine
- der Campingurlaub — camping holiday
- der Schaffner — conductor (m)
- der Ski-Urlaub — ski holiday
- der Snowboard-Urlaub — snowboard holiday
- der Ticketkauf — ticket purchase
- der Traumurlaub — dream holiday
- der Umzug — move
- der Vermieter — landlord
- die Attraktion — attraction
- die Bergtour — mountain tour
- die Burg — castle
- die Kontonummer — account number
- die Metzgerei — butcher
- die Pension — guesthouse
- die Schaffnerin — conductor (f)
- die Stadttour — city tour
- die Taxifahrt — taxi ride
- die Tour — tour
- die Vermieterin — landlady
- die Wohnfläche — living space
- die Wohnungsanzeige — flat advert
- die Wohnungssuche — flat search
- maximal — maximum
- orientieren (sich) — to orient oneself
- qm (Quadratmeter) — sq m
- vermieten — to rent out
- zurückfinden — to find way back
- überweisen — to transfer
**Reference / System**

- die Kurzform — short form
- die Ortsangabe — location

## Chapter 40 — Verben mit Dativ  ·  62 words

**Core Active**

- danken — to thank
- das Hemd — shirt
- das Kleid — dress
- das T-Shirt — t-shirt
- der Pullover — jumper
- der Schuh — shoe
- die Hose — trousers
- die Jacke — jacket
- die Kleidung — clothing
- gefallen — to please/like
- gehören — to belong
- helfen — to help
- passen (fit) — to fit
- tragen — to wear
**Active Support**

- anprobieren — to try on
- anziehen — to put on
- aussehen — to look (appear)
- ausziehen — to take off
- bequem — comfortable
- das Sweatshirt — sweatshirt
- der Anzug — suit
- der Handschuh — glove
- der Hut — hat
- der Mantel — coat
- der Pulli — pullover
- der Regenschirm — umbrella
- der Rock — skirt
- der Schal — scarf
- der Sportschuh — trainer
- der Stiefel — boot
- die Bluse — blouse
- die Handtasche — handbag
- die Hilfe — help
- die Jeans — jeans
- die Krawatte — tie
- die Mütze — hat (cap)
- die Regenjacke — rain jacket
- die Seife — soap
- die Sonnenbrille — sunglasses
- die Sonnencreme — sunscreen
- die Tasche — bag
- die Winterjacke — winter jacket
- eng — tight
- fehlen — to be missing
- freuen (sich) — to look forward
- ohne (+A) — without
- packen — to pack
- passen — to fit/match
- stehen (suit) — to suit
- viel zu — much too
- weit (loose) — loose
**Passive**

- das Kofferpacken — packing
- das Tuch — shawl/cloth
- das Zelt — tent
- der Badeanzug — swimsuit
- der Bikini — bikini
- der Helm — helmet
- der Schlafsack — sleeping bag
- die Badehose — swimming trunks
- die Klamotten — clothes (slang)
- die Reisetasche — travel bag
**Reference / System**

- der Gefallen — approval

## Chapter 41 — Dativpronomen  ·  2 words

**Core Active**

- dir — to you (dat.)
- Ihnen — to you (formal dat.)

## Chapter 42 — Welcher & Dieser  ·  87 words

**Core Active**

- diese/dieser — this
- welche/welcher — which
**Active Support**

- auf sein — to be open
- das Gramm — gram
- das Kaufhaus — department store
- das Kilo — kilo
- das Kilogramm — kilogram
- das Kleidergeschäft — clothes shop
- das Papier — paper
- das Produkt — product
- der Becher — cup/tub
- der Bleistift — pencil
- der Block — notepad/block
- der Buchladen — bookshop
- der Einkauf — shopping
- der Haushalt — household
- der Laden — shop
- der Liter — litre
- der Schuhladen — shoe shop
- der Shop — shop
- die Dose — can/jar
- die Größe — size/height
- die Kamera — camera
- die Kasse — checkout
- die Mode — fashion
- die Packung — packet
- die Tüte — bag
- die Uhr (clock) — clock/watch
- die Zeitschrift — magazine
- die Öffnungszeiten — opening hours
- geschlossen — closed
- geöffnet — open
- kriegen — to get
- online — online
- schließen — to close
- schließen (Geschäft) — to close
- shoppen — to shop
- shoppen gehen — to go shopping
- umtauschen — to exchange
- zu sein — to be closed
- öffnen — to open
- öffnen (Bäckerei) — to open
**Passive**

- das Design — design
- das Duschgel — shower gel
- das Elektrogerät — electrical device
- das Fitnessgerät — fitness equipment
- das Fotozubehör — photo accessories
- das Gadget — gadget
- das Label — label/brand
- das Maß — measure
- das Modell — model
- das Parfüm — perfume
- das Untergeschoss — basement
- der Deckel — lid
- der Designfan — design fan
- der Flagship-Store — flagship store
- der Kugelschreiber — ballpoint pen
- der Modefan — fashionista
- der Schmuck — jewellery
- der Schuhfan — shoe fan
- der Secondhand-Laden — secondhand shop
- der USB-Stick — USB stick
- die Abendkleidung — evening wear
- die Bademode — swimwear
- die CD — CD
- die Damenmode — women's fashion
- die Erfindung — invention
- die Freizeitkleidung — casual wear
- die Haushaltswaren — household goods
- die Herrenmode — men's fashion
- die Kosmetik — cosmetics
- die Parfümerie — perfume shop
- die Saison — season
- die Schreibwaren — stationery
- die Spielwaren — toys
- die Sportkleidung — sportswear
- die Tafel (board) — board/bar
- die Technik — technology
- die Variation — variation
- die Verpackung — packaging
- die Ware — goods
- topaktuell — cutting-edge
- zuhaben — to be closed
- zurückschicken — to send back
**Reference / System**

- der Demonstrativartikel — demonstrative article
- der Interrogativartikel — interrogative article
- der Kleiderkauf — clothes shopping

## Chapter 43 — Small Talk & man  ·  105 words

**Core Active**

- das Wetter — weather
- der Regen — rain
- die Sonne — sun
- man — one (impersonal)
- regnen — to rain
- sonnig — sunny
**Active Support**

- also (well) — well
- anders — different
- auf jeden Fall — in any case
- bestimmt (Person) — certain
- bewölkt — cloudy
- das Fernsehen — television
- das Gesprächsthema — conversation topic
- das Leben — life
- das Thema — topic
- der Berg — mountain
- der Grad — degree
- der Nachteil — disadvantage
- der Norden — north
- der Osten — east
- der Schnee — snow
- der Small Talk — small talk
- der Süden — south
- der Traum — dream
- der Vorschlag — suggestion
- der Vorteil — advantage
- der Westen — west
- der Wetterbericht — weather report
- der Wind — wind
- die Entscheidung — decision
- die Geschichte — story
- die Himmelsrichtung — compass direction
- die Kultur — culture
- die Kälte — the cold
- die Landschaft — landscape
- die Lust — desire/feel like
- die meisten — most
- die Nachrichten — news
- die Natur — nature
- die Politik — politics
- die Religion — religion
- die Rolle — role
- die Serie — series
- die Welt — world
- die Wolke — cloud
- doch (Partikel) — indeed
- echt (Echt?) — really?
- egal — whatever
- gerade — at the moment
- heiß — hot
- interessieren — to interest
- jemand — someone
- kaum — barely
- kennenlernen — to get to know
- klar — of course
- klar (Alles klar) — all good
- leben — to live
- los sein — to be happening
- mal — sometime/just
- meinen — to think/mean
- negativ — negative
- oder (right?) — right?
- positiv — positive
- reden — to talk
- schauen — to look
- scheinen — to shine
- schneien — to snow
- schon wieder — yet again
- selbst — oneself
- so (Zufall) — such
- total — totally
- wieder — again
- windig — windy
- zum Glück — luckily
- überall — everywhere
**Passive**

- aktuell — current
- das Mistwetter — awful weather
- das Pech — bad luck
- das Studio — studio
- dazu — to that
- denn (Partikel) — then (particle)
- der Individualist — individualist (m)
- der Mist — bother/darn
- der Quatsch — nonsense
- der Zufall — coincidence
- die Atmosphäre — atmosphere
- die Begeisterung — enthusiasm
- die Heimat — homeland
- die Individualistin — individualist (f)
- die Medien — media
- die Radiosendung — radio show
- die Reaktion — reaction
- die Szene — scene
- die Äußerung — remark
- diskutieren — to discuss
- echt — really
- eigentlich — actually
- gar — really/at all
- ja (Modalpartikel) — (modal particle)
- klingen — to sound
- nerven — to annoy
- neutral — neutral
- schrecklich — terrible
- schütten — to pour (rain)
**Reference / System**

- das Missfallen — disapproval

## Chapter 44 — Imperativ & Modalverben 2  ·  106 words

**Core Active**

- das Bein — leg
- das Fieber — fever
- das Medikament — medicine
- das Rezept — prescription
- der Bauch — stomach
- der Fuß — foot
- der Hals — neck/throat
- der Kopf — head
- der Körper — body
- der Rücken — back
- der Schmerz — pain
- die Hand — hand
- die Kopfschmerzen — headache
- die Tablette — pill
- dürfen — may/allowed
- gesund — healthy
- husten — to cough
- krank — sick
- sollen — should
**Active Support**

- aktiv — active
- bewegen — to move
- das Alter — age
- das Auge — eye
- das Gesicht — face
- das Haar — hair
- das Knie — knee
- das Ohr — ear
- der Arm — arm
- der Arztbesuch — doctor's visit
- der Doktor — doctor (m)
- der Finger — finger
- der Mund — mouth
- der Patient — patient (m)
- der Unfall — accident
- der Zahn — tooth
- der Zahnarzt — dentist (m)
- die Bauchschmerzen — stomach ache
- die Bewegung — exercise/movement
- die Bitte — request
- die Doktorin — doctor (f)
- die Halsschmerzen — sore throat
- die Krankheit — illness
- die Nase — nose
- die Patientin — patient (f)
- die Rückenschmerzen — back pain
- die Zahnschmerzen — toothache
- die Zahnärztin — dentist (f)
- erkältet — having a cold
- erlaubt — allowed
- erlaubt sein — to be allowed
- fit — fit
- geben (Anweisung) — to give
- geben (Gib alles) — to give (your all)
- gegen (+A) — for/against
- gute Besserung — get well soon
- holen — to get/fetch
- holen (Hol Wasser) — to get
- kontrollieren — to check
- leise — quiet
- nehmen (Saft) — to take (med.)
- rauchen — to smoke
- ruhig — quiet/calm
- schaffen — to manage
- schlecht (Mir ist) — feel sick
- sich — oneself
- sofort — immediately
- sportlich — athletic
- untersuchen — to examine
- verletzt — injured
- versuchen — to try
- wehtun — to hurt
**Passive**

- baden — to bathe
- das Blut — blood
- das Gewicht — weight
- das Hausmittel — home remedy
- das Pflaster — plaster
- das Suppenhuhn — soup chicken
- der Forumsbeitrag — forum post
- der Honig — honey
- der Hustensaft — cough syrup
- der Saft (Hustensaft) — syrup
- der Schnupfen — head cold
- der Tropfen — drop
- der Verband — bandage
- der Zettel — note slip
- der/die Verletzte — injured person
- die Anweisung — instruction
- die Hühnersuppe — chicken soup
- die Kontrolle — check-up
- die Nelke — clove
- die Physiotherapie — physiotherapy
- die Salbe — ointment
- die Schlaftablette — sleeping pill
- die Spritze — injection
- die Wunde — wound
- hinlegen (sich) — to lie down
- inhalieren — to inhale
- riechen — to smell
- verboten — forbidden
- vermuten — to suspect
- wiegen — to weigh
**Reference / System**

- das Gebot — command
- das Verbot — prohibition
- der Körperteil — body part
- die Aufforderung — request/command
- die Erlaubnis — permission

## Chapter 45 — Telefonieren & Nachfragen  ·  34 words

**Core Active**

- auf Wiederhören — goodbye (phone)
- der Anruf — phone call
- telefonieren — to telephone
- verbinden — to connect
- verbinden (mit) — to connect
- wie bitte — pardon?
**Active Support**

- ausdrucken — to print out
- besetzt — busy/engaged
- das Internet — internet
- das Netz — reception/net
- das Passwort — password
- das Telefongespräch — phone call
- das WLAN — Wi-Fi
- der Akku — battery
- der Laptop — laptop
- die App — app
- die Sprachnachricht — voice message
- melden (sich) — to answer (phone)
- nachfragen — to ask again
- noch mal — again
- schicken — to send
- wiederholen — to repeat
**Passive**

- das Computerproblem — computer problem
- der Anschluss — connection
- der Drucker — printer
- der Handy-Akku — phone battery
- die Datei — file
- die Durchwahl — extension
- die Fitness-App — fitness app
- drucken — to print
- eingeben — to enter (input)
- hochfahren — to boot up
- offline — offline
- speichern — to save

## Chapter 46 — Zeitangaben mit Dativ  ·  25 words

**Core Active**

- ab (+D) — from (time on)
**Active Support**

- ab (off) — off (we're off)
- alle (every) — every
- an (+D) — over (period)
- bald — soon
- danach — afterwards
- das Ende — end
- dauern — to last
- der Anfang — beginning
- der Schluss — end
- die Hitze — heat
- einmal — once
- enden — to end
- endlich — finally
- erst — only/not until
- her — ago
- letzte/letzter — last
- nur noch — only
- später — later
- stattfinden — to take place
- vorbei sein — to be over
- vorher — before
- zum Schluss — at the end
**Passive**

- außerhalb — outside of
- bis zu — up to

## Chapter 47 — Goethe A1 Final  ·  33 words

**Active Support**

- auswendig — by heart
- der Fehler — mistake
- der Punkt (Punkte geben) — point
- der Test — test
- der Tipp — tip
- die Aufgabe — task
- die Liste — list
- die Strategie — strategy
- die Übung — exercise
**Passive**

- ausdrücken — to express
- das Deutsche — German (lang.)
- das Ergebnis — result
- das Ziel — goal
- dazugehören — to belong to
- der Abschnitt — section
- der Anhang — appendix
- der Englisch-Test — English test
- der Infotext — info text
- der Mathe-Test — maths test
- der Start — start
- die Überschrift — heading
- erschließen — to deduce
- formulieren — to phrase
- korrigieren — to correct
- weiterhören — to keep listening
- wiedergeben — to reproduce
- zerlegen — to break down
**Reference / System**

- das Kompositum — compound word
- der Ausdruck — expression
- der Kontext — context
- der Stichpunkt — bullet point
- der Textbaustein — text block
- die Wortliste — word list
